...
// ===== Injetar CSS (otimizado com logo e adições para vibe adulto, tons terrosos) =====
...
.notif-dot{position:absolute;top:-6px;right:-6px;background:linear-gradient(135deg,#AB865B,#D3AD83);color:#fff;font-size:11px;width:18px;height:18px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-weight:600;box-shadow:0 2px 8px rgba(142,110,74,0.4);animation:pulse 2s infinite}
...
.notif-item button{background:linear-gradient(135deg,#AB865B,#D3AD83);border:none;border-radius:20px;padding:10px 20px;color:#fff;font-size:14px;cursor:pointer;font-weight:600;transition:all .3s;width:100%;box-shadow:0 4px 12px rgba(142,110,74,0.3)}
.notif-item button:hover{opacity:0.9;transform:translateY(-1px);box-shadow:0 6px 16px rgba(142,110,74,0.4)}
.notif-item button:active{transform:translateY(0);box-shadow:0 2px 8px rgba(142,110,74,0.2)}
.popup-footer{font-size:12px;color:#aaa;text-align:center;margin-top:8px;letter-spacing:0.3px}
.toast{position:fixed;top:20px;right:20px;background:linear-gradient(135deg,#AB865B,#D3AD83);color:#fff;padding:12px 20px;border-radius:20px;box-shadow:0 8px 24px rgba(142,110,74,0.4);transform:translateX(400px);opacity:0;transition:all .4s cubic-bezier(0.25,0.46,0.45,0.94);z-index:10001;font-size:14px;font-weight:500;min-width:200px;text-align:center}
.toast.active{transform:translateX(0);opacity:1}
.toast.exit{transform:translateX(400px);opacity:0}
.scroll-top{position:fixed;bottom:30px;right:30px;width:50px;height:50px;background:linear-gradient(135deg,#AB865B,#D3AD83);border-radius:50%;display:none;align-items:center;justify-content:center;cursor:pointer;transition:all .3s cubic-bezier(0.25,0.46,0.45,0.94);box-shadow:0 8px 24px rgba(142,110,74,0.4);z-index:9998}
...
.teaser-popup img{width:100%;max-height:250px;object-fit:cover;border-radius:24px;border:1px solid rgba(255,255,255,0.2);backdrop-filter:blur(2px);margin-bottom:12px;transition:all .3s;}
.teaser-popup button{padding:12px 24px;height:48px;}
.teaser-popup button:hover{opacity:0.9;transform:translateY(-2px);box-shadow:0 8px 20px rgba(142,110,74,0.5);}
.teaser-popup button:active{transform:translateY(0);box-shadow:0 4px 12px rgba(142,110,74,0.3);}
.loader-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(32,32,32,0.9);display:none;align-items:center;justify-content:center;z-index:10002;backdrop-filter:blur(4px);}
.loader-spinner{width:40px;height:40px;border:4px solid rgba(255,255,255,0.1);border-top:4px solid #D3AD83;border-radius:50%;animation:spin 1s linear infinite;}
@keyframes spin{0%{transform:rotate(0deg);}100%{transform:rotate(360deg);}}
@media (max-width:768px){.teaser-popup button{height:44px;padding:10px 20px;}}
`;
document.head.appendChild(style);

// ===== Injetar HTML (com logo no canto esquerdo e adições, sem busca) =====
const navHTML = `
...
<div class="overlay" id="notifOverlay" role="dialog" aria-modal="true" aria-labelledby="notifTitle">
  <div class="popup" id="notifPopup">
    ...
    <div class="notif-item">
      ...
      <button id="premiumBtn1">Quero ser Premium</button>
    </div>
    <div class="notif-item">
...
<div class="overlay" id="teaserOverlay" role="dialog" aria-modal="true" aria-labelledby="teaserTitle">
  <div class="popup teaser-popup" id="teaserPopup">
    ...
    <p>Assine Premium para ver sem censura e acessar vídeos exclusivos!</p>
    <button id="premiumBtn2">Assinar agora - 20% off</button>
    <div class="popup-footer">Oferta limitada! ⏰</div>
  </div>
</div>
<div class="overlay" id="muralOverlay" role="dialog" aria-modal="true" aria-labelledby="muralTitle">
  <div class="popup" id="muralPopup">
    <button class="close-popup" id="closeMural" aria-label="Fechar mural info">&times;</button>
    <h2 id="muralTitle">Bem-vindo ao Mural!</h2>
    <div class="notif-item">
      <p>Você está no mural de modelos. Explore e descubra conteúdos incríveis!</p>
      <p>Mas para ver o catálogo completo, acesse agora.</p>
      <button onclick="window.location.href='https://frameag.com/models'">Ver Catálogo Completo</button>
    </div>
    <div class="popup-footer">Frame Mural | Atualizado Hoje</div>
  </div>
</div>
...
function showLoaderThenRedirect(url) {
  ...
  }, 1500); // 1.5s delay para efeito premium
}
...
favIcon?.addEventListener('click', () => {
  ...
  if (filled) {
    showToast('FrameMural favoritado! Exibiremos mais resultados como este');
  } else {
    showToast('Removido dos favoritos. Vamos ajustar as recomendações para você!');
  }
});

// Categorias icon - agora abre popup mural
...
// Premium buttons with loader
premiumBtn1?.addEventListener('click', () => showLoaderThenRedirect('https://frameag.com/premium'));
premiumBtn2?.addEventListener('click', () => showLoaderThenRedirect('https://frameag.com/premium'));
...
// Animação extra: Shake no notif-dot a cada 10s e glow aleatório nos ícones
setInterval(() => {
...
setInterval(() => {
  if (Math.random() > 0.8) {
    categoriesIcon?.classList.add('shake');
    setTimeout(() => categoriesIcon?.classList.remove('shake'), 700);
  }
}, 15000);
...
// ===== Injetar CSS (atualizado com tons terrosos, efeitos no botão, borda glass, responsividade) =====
const style = document.createElement('style');
style.textContent = `
body{margin:0;font-family:'Montserrat',sans-serif;background:linear-gradient(to bottom,#f8f8f8,#e0d0c0);height:200vh;overflow-x:hidden}
...
// ===== Injetar HTML (sem barra de busca, com mural popup, imagem atualizada, botões terrosos) =====
const navHTML = `
<div id="navMenu" role="navigation" aria-label="Menu principal">
  <div class="close-menu" id="closeMenu" aria-label="Fechar menu">&times;</div>
  <img class="logo-img" src="https://framerusercontent.com/images/JaIvmSW2LTbs0XCR7tnpcmU8xA.png" alt="Logo Frame" aria-label="Logo Frame">
...
<div class="overlay" id="notifOverlay" role="dialog" aria-modal="true" aria-labelledby="notifTitle">
  <div class="popup" id="notifPopup">
    ...
    <div class="notif-item">
      <p>Temos vídeos quentinhos para você! 🔥</p>
      <button onclick="window.location.href='/models'">Ver catálogo de modelos</button>
    </div>
    <div class="popup-footer" id="notifTime">Frame News | Hoje</div>
  </div>
</div>
<div class="overlay" id="muralOverlay" role="dialog" aria-modal="true" aria-labelledby="muralTitle">
  <div class="popup" id="muralPopup">
    ...
    <div class="notif-item">
      <p>Explore os destaques do FrameMural. Quer ver o catálogo completo?</p>
      <button onclick="window.location.href='/models'">Ver Catálogo Completo</button>
    </div>
    <div class="popup-footer">FrameMural | Hoje</div>
  </div>
</div>
<div class="overlay" id="teaserOverlay" role="dialog" aria-modal="true" aria-labelledby="teaserTitle">
  <div class="popup teaser-popup" id="teaserPopup">

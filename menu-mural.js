...
// ===== Injetar CSS (otimizado com logo e adições para vibe adulto) =====
...
.notif-dot{position:absolute;top:-6px;right:-6px;background:linear-gradient(135deg,#C04040,#FF8080);color:#fff;font-size:11px;width:18px;height:18px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-weight:600;box-shadow:0 2px 8px rgba(142,110,74,0.4);animation:pulse 2s infinite}
...
.notif-item button{background:linear-gradient(135deg,#C04040,#FF8080);border:none;border-radius:20px;padding:10px 20px;color:#fff;font-size:14px;cursor:pointer;font-weight:600;transition:all .3s;width:100%;box-shadow:0 4px 12px rgba(142,110,74,0.3)}
...
.toast{position:fixed;top:20px;right:20px;background:linear-gradient(135deg,#C04040,#FF8080);color:#fff;padding:12px 20px;border-radius:20px;box-shadow:0 8px 24px rgba(142,110,74,0.4);transform:translateX(400px);opacity:0;transition:all .4s cubic-bezier(0.25,0.46,0.45,0.94);z-index:10001;font-size:14px;font-weight:500;min-width:200px;text-align:center}
.toast.active{transform:translateX(0);opacity:1}
.toast.exit{transform:translateX(400px);opacity:0}
.scroll-top{position:fixed;bottom:30px;right:30px;width:50px;height:50px;background:linear-gradient(135deg,#C04040,#FF8080);border-radius:50%;display:none;align-items:center;justify-content:center;cursor:pointer;transition:all .3s cubic-bezier(0.25,0.46,0.45,0.94);box-shadow:0 8px 24px rgba(142,110,74,0.4);z-index:9998}
...
.filled{fill:#C04040}
@keyframes glow { 0% { filter: drop-shadow(0 0 0 rgba(255,128,128,0.7)); } 50% { filter: drop-shadow(0 0 10px rgba(255,128,128,0.7)); } 100% { filter: drop-shadow(0 0 0 rgba(255,128,128,0.7)); } }
#navMenu svg:hover, #navMenu img:not(.logo-img):hover { animation: glow 1s infinite; }
.search-wrapper{flex:1;max-width:300px;margin:0 20px;}
.search-wrapper input{width:100%;padding:8px 12px;border-radius:20px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.1);color:#fff;font-size:14px;outline:none;transition:all .3s;}
.search-wrapper input:focus{border-color:rgba(255,128,128,0.5);box-shadow:0 0 8px rgba(255,128,128,0.3);}
.teaser-popup img{width:100%;border-radius:16px;filter:blur(5px);margin-bottom:12px;transition:filter .3s;}
.teaser-popup:hover img{filter:blur(3px);}
`;
document.head.appendChild(style);

// ===== Injetar HTML (com logo no canto esquerdo e adições) =====
const navHTML = `
<div id="navMenu" role="navigation" aria-label="Menu principal">
  ...
  <div class="search-wrapper">
    <input type="text" id="searchModels" placeholder="Busque modelos..." aria-label="Busca por modelos">
  </div>
...
<div class="overlay" id="teaserOverlay" role="dialog" aria-modal="true" aria-labelledby="teaserTitle">
  <div class="popup teaser-popup" id="teaserPopup">
    ...
    <img src="https://framerusercontent.com/images/JaIvmSW2LTbs0XCR7tnpcmU8xA.png" alt="Modelo teaser" style="width:100%;border-radius:16px;filter:blur(5px);margin-bottom:12px;"> <!-- Substitua por uma imagem real borrada -->
    <p>Assine Premium para ver sem censura e acessar vídeos exclusivos!</p>
    <button onclick="window.open('https://frameag.com/premium','_blank')">Assinar agora - 20% off</button>
...
// Search placeholder (expanda para real search)
searchModels?.addEventListener('input', (e) => {
  console.log(`Buscando modelos: ${e.target.value}`); // Integre com API de busca de modelos
});

// Categorias icon
categoriesIcon?.addEventListener('click', () => {
  showToast('Explorando mural: Novas Modelos, Top Rated, Ao Vivo, etc.'); // Expanda para popup real se necessário
});
...
// ESC para fechar notificações e teaser
